...
final class NodeFrame extends javax.swing.JFrame {
    ...
    NodeFrame( final PhylogenyNode n, final Phylogeny tree, final TreePanel tp, final int x , int branchdata) {
    	super ("Branch " + ( (n.getNodeData().isHasSequence()) ? ( n.getNodeData().getSequences().get(0).getName()) : "" ));
    	  	
        _reepanel = tp;
        setSize( Constants.NODE_FRAME_SIZE );
        _index = x;
        final Container contentPane = getContentPane();
        final BranchPanel nodepanel = new BranchPanel( n );
        contentPane.add( nodepanel, BorderLayout.CENTER );
        addWindowListener( new WindowAdapter() {
