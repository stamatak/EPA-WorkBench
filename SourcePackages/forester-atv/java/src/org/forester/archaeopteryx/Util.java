...
final class Util {
    ...
    final static void colorPhylogenyAccordingToEDPL( final Phylogeny tree, final TreePanel tree_panel ,double cutoff) {
    	final Color bg = tree_panel.getTreeColorSet().getBackgroundColor();
    	final Color br = tree_panel.getTreeColorSet().getBranchColor();
    	for( final PhylogenyNodeIterator it = tree.iteratorPreorder(); it.hasNext(); ) {
    		final PhylogenyNode n = it.next();
    		// if edpl score is available
    		if (n.getNodeData() == null || !(n.getNodeData().isHasSequence())){
    			continue;
    		}
    		else if ( n.getNodeData().getSequences().get(1).getAnnotations().size() > 1 ) {
    			final double conf = PhylogenyMethods.getEDPLValue( n );
    			BranchColor current_color = n.getBranchData().getBranchColor();
    			final BranchColor c =  calcEDPLColor( conf , current_color, cutoff) ;
    			n.getBranchData().setBranchColor( c );
    		}
    	}
    }
    
    final static void unColorPhylogenyAccordingToEDPL( final Phylogeny tree, final TreePanel tree_panel, double current_cutoff ) {
    	final Color bg = tree_panel.getTreeColorSet().getBackgroundColor();
    	final Color br = tree_panel.getTreeColorSet().getBranchColor();
    	for( final PhylogenyNodeIterator it = tree.iteratorPreorder(); it.hasNext(); ) {
    		final PhylogenyNode n = it.next();
    		...
    		if ( n.getNodeData().getSequences().get(1).getAnnotations().size() > 1 ) {
    			final double conf = PhylogenyMethods.getEDPLValue( n );
    			BranchColor current_color = n.getBranchData().getBranchColor();
    			final BranchColor c =  removeEDPLColor( conf , current_color,current_cutoff) ;
    			n.getBranchData().setBranchColor( c );
    		}
    	}
    }
    
    final static BranchColor calcEDPLColor( double conf, BranchColor current_color, double cutoff){
    	Double val = 0.0;
    	if (conf > cutoff){
		...
		green = (green+current_color.getValue().getGreen())/2;
		red = 255-green;
		}
		else{
			current_color = new BranchColor();
		}
		current_color.setValue(new Color(red,green,blue));
    	return current_color;
    }  
    
    // can only be approximated and delivers wrong colors and exceptions after some time of sliding
    final static BranchColor removeEDPLColor( double conf, BranchColor current_color, double cutoff){
    	double m = 255.0 / (1.0 - cutoff);
		double b = m - 255;
		Double val = m*(conf) - b ;
//		int green = 0;
		...
		int blue = 0;
		if (current_color != null){
			green = ((2*current_color.getValue().getGreen())-green);
			red = 255 - green;
			if (red == 256){
				red = 255;
				green = 0;
    ...
    final static void colorPhylogenyAccordingToQSBI( final Phylogeny tree, final TreePanel tree_panel ) {
    	final Color bg = tree_panel.getTreeColorSet().getBackgroundColor();
    	final Color br = tree_panel.getTreeColorSet().getBranchColor();
    	for( final PhylogenyNodeIterator it = tree.iteratorPreorder(); it.hasNext(); ) {
    		final PhylogenyNode n = it.next();
    		...
    		if ( n.getNodeData().isHasSequence() ) {
    			final double conf = PhylogenyMethods.getQSBIValue( n );
    			BranchColor current_color = n.getBranchData().getBranchColor();
    			final BranchColor c = calcQSBIColor( conf , current_color) ;
    			n.getBranchData().setBranchColor( c );
    		}
    	}
    }
    
    final static void unColorPhylogenyAccordingToQSBI( final Phylogeny tree, final TreePanel tree_panel ) {
    	final Color bg = tree_panel.getTreeColorSet().getBackgroundColor();
    	final Color br = tree_panel.getTreeColorSet().getBranchColor();
    	for( final PhylogenyNodeIterator it = tree.iteratorPreorder(); it.hasNext(); ) {
    		final PhylogenyNode n = it.next();
    		...
    		if ( n.getNodeData().isHasSequence() ) {
    			final double conf = PhylogenyMethods.getQSBIValue( n );
    			boolean other_color_available = false;
    			if (n.getNodeData().getSequences().get(1).getAnnotations().size() > 1){
    				other_color_available = true;
    			}
    			BranchColor current_color = n.getBranchData().getBranchColor();
    			final BranchColor c = removeQSBIColor( conf , current_color, other_color_available);
    			n.getBranchData().setBranchColor( c );
    		}
    	}
    }
    
    final static BranchColor calcQSBIColor( double conf, BranchColor current_color){
 ...
 // can only be approximated and delivers wrong colors and exceptions after some time of sliding	
    final static BranchColor removeQSBIColor( double conf, BranchColor current_color, boolean other_color_available){
    	double m = 255.0 ;
		double b = m - 255;
		Double val = m*(conf) - b ;
		int green = val.intValue();
