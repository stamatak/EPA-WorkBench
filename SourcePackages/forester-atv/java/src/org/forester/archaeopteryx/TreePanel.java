// $Id: TreePanel.java,v 1.165 2010/04/11 00:02:25 cmzmasek Exp $
...
public final class TreePanel extends JPanel implements ActionListener, MouseWheelListener, Printable {
    ...
    final void qsbiColor(boolean state){
    	   if ( ( _phylogeny == null ) || ( _phylogeny.getNumberOfExternalNodes() < 2 ) ) {
               return;
           }
           setWaitCursor();
           if (state){
        	   Util.colorPhylogenyAccordingToQSBI( _phylogeny, this );
        	   _control_panel.setColorBranches( true );
        	   if ( _control_panel.getColorBranchesCb() != null ) {
        		   _control_panel.getColorBranchesCb().setSelected( true );
        	   }
        	   if ( _control_panel.getColorBranchesQsbi() != null) {
        		   _control_panel.getColorBranchesQsbi().setSelected(true);
        	   }
           }
           else{
        	   // if qsbi gets deseleted but edpl is selected, then remove only the qsbi colors
        	   if (_main_panel.getControlPanel().getColorBranchesEdpl().isSelected()){
        		   Util.removeBranchColors(_phylogeny);
        		   Util.colorPhylogenyAccordingToEDPL(_phylogeny, this, getControlPanel().getCurrentCutoff());
        	   }
        	   else{
        		   Util.removeBranchColors(_phylogeny);
        	   }
    ...
    final void edplColor(boolean state, double cutoff){
    	if ( ( _phylogeny == null ) || ( _phylogeny.getNumberOfExternalNodes() < 2 ) ) {
    		return;
    		}
    	setWaitCursor();
    	if (state){
    		Util.colorPhylogenyAccordingToEDPL(_phylogeny, this,cutoff);
    		_control_panel.setColorBranches( true );
    		if ( _control_panel.getColorBranchesCb() != null ) {
    			_control_panel.getColorBranchesCb().setSelected( true );
    		}
    		if ( _control_panel.getColorBranchesEdpl() != null) {
    			_control_panel.getColorBranchesEdpl().setSelected(true);
    		}
    	}
    	else{
    		// if edpl gets deseleted but qsbi is selected, then remove only the edpl colors
    		if (_main_panel.getControlPanel().getColorBranchesQsbi().isSelected()){
    			Util.removeBranchColors(_phylogeny);
    			Util.colorPhylogenyAccordingToQSBI(_phylogeny, this);
    		}
    		else{
    ...
    final void edplSliderMovement(double next_cutoff, double current_cutoff){
    	Util.removeBranchColors(_phylogeny);
    	if (getControlPanel().getColorBranchesQsbi().isSelected())
    	{
    		Util.colorPhylogenyAccordingToQSBI(_phylogeny, this);
    	}
    	Util.colorPhylogenyAccordingToEDPL(_phylogeny, this,next_cutoff);
    	setArrowCursor();
        repaint();
    }
    ...
    final private void displayBranchPopupMenu( final PhylogenyNode node, final int x, final int y ) {
        makeBranchPopupMenus( node );
        _node_popup_menu.putClientProperty( NODE_POPMENU_NODE_CLIENT_PROPERTY, node );
        _node_popup_menu.show( this, x, y );
    }
    ...
    final PhylogenyNode findNode( final int x, final int y ) {
        ...
        for( final PhylogenyNodeIterator iter = _phylogeny.iteratorPostorder(); iter.hasNext(); ) {
            ...
            if ( ( _phylogeny.isRooted() || !node.isRoot() || ( node.getNumberOfDescendants() > 2 ) )
                    ...
                    && ( ( node.getYcoord() + HALF_BOX_SIZE_PLUS_WIGGLE ) >= y ) ) {
            	//System.out.printf("x: %s, y: %s", node.getXcoord(), node.getYcoord());
                return node;
            }
        }
        return null;
    ...
    final PhylogenyNode findBranch( final int x, final int y ) {
        ...
        if (getPhylogenyGraphicsType() == PHYLOGENY_GRAPHICS_TYPE.TRIANGULAR){
        	for( final PhylogenyNodeIterator iter = _phylogeny.iteratorPostorder(); iter.hasNext(); ) {
        		...
        		if ( ( _phylogeny.isRooted() || !node.isRoot() || ( node.getNumberOfDescendants() > 2 ) )
                 		&& (!node.isRoot())
                 		&& coordinatesLieOnTriangularBranch(node.getXcoord(),node.getYcoord(),node.getParent().getXcoord(),node.getParent().getYcoord(),x,y)  ){
        			return node;
            	}
            }
        return null;	
        	...
        	for( final PhylogenyNodeIterator iter = _phylogeny.iteratorPostorder(); iter.hasNext(); ) {
        		...
        		if ( ( _phylogeny.isRooted() || !node.isRoot() || ( node.getNumberOfDescendants() > 2 ) )
                 		&& (!node.isRoot())
                 		&& coordinatesLieOnRectangularBranch(node.getXcoord(),node.getYcoord(),node.getParent().getXcoord(),node.getParent().getYcoord(),x,y)  ){
        			return node;
            	}
            }
        }
        	...
        	for( final PhylogenyNodeIterator iter = _phylogeny.iteratorPostorder(); iter.hasNext(); ) {
        		...
        		if ( ( _phylogeny.isRooted() || !node.isRoot() || ( node.getNumberOfDescendants() > 2 ) )
                 		&& (!node.isRoot())
                 		&& coordinatesLieOnEuroTypeBranch(node.getXcoord(),node.getYcoord(),node.getParent().getXcoord(),node.getParent().getYcoord(),x,y)  ){
        			return node;
            	}
            }
        }
    ...
    private boolean coordinatesLieOnTriangularBranch(float x1, float y1, float x2, float y2, int x_location, int y_location){
    	//System.out.printf("x1: %s, y1: %s, x2: %s, y2: %s, loci_x: %s, loci_y: %s \n", x1,y1,x2,y2,x_location,y_location);
    	
    	float tolerance = 5;
    	float m = Math.abs(y2-y1)/Math.abs(x1-x2);
    	float xt = (float)x_location;
    	float yt = (float)y_location;
    	
    	if (x1 < x2 && y1 < y2 && xt > x1 && xt < x2 && yt > y1 && yt < y2){
    		return toleranceZone((m)*(xt-x1)+y1,yt, tolerance);   		
    	}
    	else if (x1 > x2 && y1 < y2 && xt < x1 && xt > x2 && yt > y1 && yt < y2){
    		return toleranceZone((-1)*m*(xt-x2)+y2,yt, tolerance);
    	}
    	else if (x1 > x2 && y1 > y2 && xt < x1 && xt > x2 && yt < y1 && yt > y2){
    		return toleranceZone(m*(xt-x2)+y2,yt, tolerance);
    	}
    	else if (x1 < x2 && y1 > y2 && xt > x1 && xt < x2 && yt < y1 && yt > y2){
    		return toleranceZone((-1)*m*(xt-x1)+y1,yt, tolerance);
    	}
    	else if (x1 < x2 && y1 == y2 && xt > x1 && xt < x2){
    		return (toleranceZone(y1,yt,tolerance)&& xt >= x1 && xt <= x2);
    	}
    	else if (x1 > x2 && y1 == y2 && xt < x1 && xt > x2){
    		return (toleranceZone(y1,yt,tolerance)&& xt <= x1 && xt >= x2);
    	}
    	else if (x1 == x2 && y1 < y2 && yt > y1 && yt < y2){
    		return (toleranceZone(x1,xt,tolerance)&& yt >= y1 && yt <= y2);
    	}
    	else if (x1 == x2 && y1 > y2 && yt < y1 && yt > y2){
    		return (toleranceZone(x1,xt,tolerance)&& yt <= y1 && yt >= y2);
    	}
    ...
    private boolean coordinatesLieOnRectangularBranch(float x1, float y1, float x2, float y2, int x_location, int y_location){
    	float tolerance = 5;
    	float xt = (float)x_location;
    	float yt = (float)y_location;
    	// top rectangular lines
    	if ((xt > x2+tolerance && xt < x1 && yt < y1+tolerance & yt > y1-tolerance) || (xt > x2-tolerance && xt < x2+tolerance && yt > y1 -tolerance && yt < y2)){
    		return true;
    	}
    	// bottom rectangular lines
    	else if ((xt > x2+tolerance && xt < x1 && yt > y2-tolerance && yt < y2+tolerance)||(xt > x2-tolerance && xt < x2+tolerance && yt > y2 && yt < y1+tolerance)){
    		return true; 		
    	}
    ...
    private boolean coordinatesLieOnEuroTypeBranch(float x1, float y1, float x2, float y2, int x_location, int y_location){
    	float tolerance = 5;
    	float xt = (float)x_location;
    	float yt = (float)y_location;
    	...
    	float zy = y1;
    	// Euro Type lines
    	if ((xt > x2-tolerance+EURO_D && xt < x1 && yt > y1-tolerance && yt < y1+tolerance) || (coordinatesLieOnTriangularBranch(zx,zy,x2,y2,x_location,y_location))){
    		return true;
    	}
    	return false;
    }
    
    private boolean toleranceZone(float should, float is, float tolerance){
    	if (is > should+tolerance || is < should-tolerance){
    		return false;   		
    	}
    	else{
    		return true;
    ...
    final private void handleClickToAction( final NodeClickAction action, final PhylogenyNode node ) {
        switch ( action ) {
            ...
            case EDIT_NODE_DATA:
                ...
                break;
            case SHOW_BRANCH_DATA:
            	showBranchFrame( node );
            	break;
    ...
    final private void makePopupMenus( final PhylogenyNode node ) {
        ...
        for( int i = 0; i < clickto_names.size(); i++ ) {
          ...
          //}
            ...
            if ( !title.equals( Configuration.clickto_options[ Configuration.reroot ][ 0 ] )  && !title.equals(Configuration.clickto_options[Configuration.display_branch_data][0])) {
    ...
    final void mouseClicked( final MouseEvent e ) {
        ...
        else if(findNode( e.getX(), e.getY() )!= null)
        {
        	...
        	if ((_control_panel.getActionWhenNodeClicked() == NodeClickAction.SHOW_BRANCH_DATA) || (_control_panel.getActionWhenNodeClicked() == NodeClickAction.SHOW_DATA)){     
    ...
    final void mouseMoved( final MouseEvent e ) {
        ...
        if ( inOv( e ) && getOptions().isShowOverview() && isOvOn() ) {
            ...
            else if ( ( branch != null ) && ( branch.isRoot() || !branch.getParent().isCollapse() ) ) {
                // cursor is over a tree node
                if ( ( getControlPanel().getActionWhenNodeClicked() == NodeClickAction.CUT_SUBTREE )
                        || ( getControlPanel().getActionWhenNodeClicked() == NodeClickAction.COPY_SUBTREE )
                        || ( getControlPanel().getActionWhenNodeClicked() == NodeClickAction.PASTE_SUBTREE )
            ...
            if ( ( getOptions().isShowNodeBoxes() && !to_pdf && !to_graphics_file )
                ...
                if ( to_pdf || to_graphics_file ) {
                	...
                	if (node.getNodeData().isHasSequence()){
                		...
                		double most_inserts = Double.valueOf(((Annotation)node.getNodeData().getSequences().get(0).getAnnotation(0)).getDesc().split(",")[0]);
                		...
                		for (int i = 1; i<descendants.size(); i++){ // first node does not get into account, because we want to only the branches after the node to count
                			inserts = inserts + ((double)descendants.get(i).getNodeData().getSequences().size())-1.0;  //first sequence tag is always the histogram	
                		}		
...
//                _sb.append( node.getNodeData().getSequence().getName() );
            	...
            	if (getPhylogenyGraphicsType() == PHYLOGENY_GRAPHICS_TYPE.TRIANGULAR){           	
            		...
            		TreePanel.drawString(node.getNodeData().getSequence().getName() , center_of_branch_x,center_of_branch_y-above_the_line, g );
                }
            	else if (getPhylogenyGraphicsType() == PHYLOGENY_GRAPHICS_TYPE.RECTANGULAR){
            		double center_of_branch_x = Math.abs(x1+x2)/2.0;
...
//                _sb.append( node.getNodeData().getSequence().getName() );
    ...
    //### Show Histogram of RAxML Weights for each Branch
    final private void showBranchDataPopup( final MouseEvent e, final PhylogenyNode node ) {
        try {
                ...
                if ( !(((Annotation)node.getNodeData().getSequences().get(0).getAnnotation(0)).getDesc().length() < 10)) { // as long as there are no branch names longer than 9 characters, this is going to work
                    lines++;
                    	_popup_buffer.delete(0,_popup_buffer.length());
                    	 histdata = ((Annotation)node.getNodeData().getSequences().get(0).getAnnotation(0)).getDesc().split(",");
                    	_popup_buffer.append("RAxml Weights Histogram "+node.getNodeData().getSequence(0).getName()+"\n");
                ...
                if ( _popup_buffer.length() > 0 ) {
                    ...
                    _chart_panel = new ChartPanel(histogram, 300, 200, ChartPanel.DEFAULT_MINIMUM_DRAW_WIDTH, ChartPanel.DEFAULT_MINIMUM_DRAW_HEIGHT, ChartPanel.DEFAULT_MAXIMUM_DRAW_WIDTH, ChartPanel.DEFAULT_MAXIMUM_DRAW_HEIGHT, ChartPanel.DEFAULT_BUFFER_USED, false, false, false, false, true);
    ...
    private static JFreeChart createChart(CategoryDataset dataset, String branch_name) {
        ...
        JFreeChart chart = ChartFactory.createBarChart(
            "RAxML Weights Histogram "+branch_name,         // chart title
        ...
        final BarRenderer renderer = new CustomRenderer( new Paint[] {new Color(255,0,0), new Color(227,28,0), new Color(199,56,0), new Color(171,84,0), new Color(143,112,0), new Color(115,140,0), new Color(87,168,0), new Color(59,196,0), new Color(31,224,0), new Color(0,255,0)});
    ...
    private CategoryDataset createDataset(String[] histdata) {
    	DefaultCategoryDataset dataset = new DefaultCategoryDataset();
		// row keys
		String series1 = "First";

		// column keys
		String[] categories = {"0.0-0.1","0.1-0.2","0.2-0.3","0.3-0.4","0.4-0.5","0.5-0.6","0.6-0.7","0.7-0.8","0.8-0.9","0.9-1.0"};

		// Parse the bar values
		Pattern p = Pattern.compile("\\s*\\d\\.\\d\\s-\\s\\d\\.\\d:\\s[\\|\\.]*\\s*(\\d+)");
		ArrayList <Double> data = new ArrayList<Double>() ;
    	for (int i = 0; i < histdata.length; i++){
    		Matcher m = p.matcher(histdata[i]);
    		if (m.matches()){
    			data.add( Double.valueOf(m.group(1)));
    		}   
    	}
		for (int i = 0; i < categories.length; i++){
			dataset.addValue(data.get(i), series1, categories[i]);
		}
		return dataset;
	}
    ...
    final private void showBranchFrame( final PhylogenyNode n ) {
        if ( _node_frame_index < TreePanel.MAX_NODE_FRAMES ) {
            // pop up edit box for single node
        	if (n.getNodeData().isHasSequence()){
        		_node_frames[ _node_frame_index ] = new NodeFrame( n, _phylogeny, this, _node_frame_index,0 );
        		_node_frame_index++;
        	}
        }
        else {
